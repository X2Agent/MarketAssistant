...
public interface ICryptoBasicTools : IBasicDataTools
{
    ...
    /// </summary>
    Task<CryptoQuoteInfo> GetAssetInfoAsync(string assetSymbol, CancellationToken cancellationToken = default);

    /// <summary>
    /// 根据虚拟币代码获取区块链项目基本面信息，包括项目简介、社区数据、开发者活跃度等
    /// </summary>
    Task<CryptoProjectInfo> GetProjectInfoAsync(string assetSymbol, CancellationToken cancellationToken = default);
}
