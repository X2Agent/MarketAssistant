...
public sealed class AShareBasicTools : IShareBasicTools
{
    ...
    public AShareBasicTools(IHttpClientFactory httpClientFactory, IUserSettingService userSettingService)
    {
    ...
    public async Task<CompanyInfo> GetCompanyInfoAsync([Description("股票代码")] string assetSymbol)
    {
        try
        {
            ...
            var url = $"https://api.zhituapi.com/hs/gs/gsjj/{assetSymbol}?token={token}";
