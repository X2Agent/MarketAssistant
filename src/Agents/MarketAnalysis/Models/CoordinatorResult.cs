...
public sealed class CoordinatorResult
{
    ...
    /// </summary>
    [Range(0, 100)]
    [Description("置信度百分比，0-100，基于信息完整性和一致性。评分标准：" + ScoringStandards.Reliability)]
    public float ConfidencePercentage { get; set; }
    
    /// <summary>
    /// 各维度评分
    /// </summary>
    [Description("各维度评分字典，必须包含：基本面、技术面、市场情绪、新闻事件四个维度，每个维度评分范围1-10。评分标准：" + ScoringStandards.Quality)]
    ...
    [MinLength(1)]
    [MaxLength(3)]
    [Description("1-3个最核心的投资驱动因素，结合最新信息，避免空洞描述")]
    ...
    [MinLength(1)]
    [MaxLength(3)]
    [Description("1-3个最主要的潜在风险，包含新识别的风险")]
    ...
    [MinLength(3)]
    [MaxLength(5)]
    [Description("3-5条具体可执行的操作建议，整合各分析师建议，提供具体方案（入场点、止损位、仓位管理）。例如：'建议在 45-48 元区间分批建仓（技术支撑+估值合理）'")]
    ...
    [MinLength(50)]
    [MaxLength(300)]
    [Description("100-200字，总结所有分析师一致认同的关键观点")]
    ...
    [MinLength(50)]
    [MaxLength(300)]
    [Description("100-200字，明确指出分歧点和你的综合判断，如使用了搜索工具需说明搜索结果及其影响。例如：'基本面看涨20%，技术面看涨5%。经搜索验证，市场已提前反应业绩预期，综合判断为上涨10-12%'")]
    ...
    [MinLength(6)]
    [MaxLength(10)]
    [Description("6-10个从各专业分析师的自然语言分析中提取的最关键指标和数据点，数据具体、判断清晰、建议可行")]
