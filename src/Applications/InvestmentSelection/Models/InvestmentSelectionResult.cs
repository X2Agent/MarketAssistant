...
/// 投资选择结果
/// </summary>
[Description("投资选择分析结果，包含推荐投资标的列表和分析报告")]
public class InvestmentSelectionResult
{
    /// <summary>
    /// 选择类型
    /// </summary>
    [Description("选择类型：user_request(用户需求分析) 或 news_based(新闻分析)")]
    ...
    /// 推荐投资标的列表
    /// </summary>
    [Description("推荐投资标的列表，按推荐优先级从高到低排序。若无合适标的则返回空数组[]")]
    public List<InvestmentRecommendation> Recommendations { get; set; } = new();
