    ...
    public async Task<bool> AddToFavoriteAsync(object stockParameter)
    {
        try
        {
            if (stockParameter is HotStock hotStock)
            {
                ...
                await _dialogService.ShowAlertAsync("收藏成功", $"已将 {hotStock.Name} 添加到收藏列表", "确定");
                return true;
            }
            else if (stockParameter is StockItem stockItem)
            {
                ...
                await _dialogService.ShowAlertAsync("收藏成功", $"已将 {stockItem.Name} 添加到收藏列表", "确定");
                return true;
            }
        ...
        catch (Exception ex)
        {
            ...
            await _dialogService.ShowAlertAsync("收藏失败", "添加收藏时发生错误，请稍后重试", "确定");
            return false;
        }
    }
}
